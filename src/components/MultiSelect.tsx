      ...
      <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen} modal={modalPopover}>
          ...
          >
            {selectedValues.length > 0 ? (
              ...
              <div className='flex items-center justify-between w-full mx-auto'>
                <span className='text-sm font-normal text-zinc-500'>{placeholder}</span>
                <ChevronDown className='h-4 cursor-pointer text-zinc-300 dark:text-zinc-500' />
              </div>
            )}
          </Button>
        ...
        <PopoverContent className='w-auto p-0' align='start' onEscapeKeyDown={() => setIsPopoverOpen(false)}>
          <Command shouldFilter={!async}>
            ...
            />
            <CommandList>
              {async && error && <div className='p-4 text-destructive text-center'>{error.message}</div>}
              ...
              {async ? (
                ...
                !error &&
                options.length === 0 && (
                  <div className='pt-6 pb-4 text-center text-sm'>{`No ${placeholder.toLowerCase()} found.`}</div>
                )
              ) : (
